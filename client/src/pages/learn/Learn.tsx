...
const CourseCard: React.FC<{
  ...
  // Calculate logic based on course data
  const isCompleted = course.progress !== undefined && course.progress > 0;
  ...
  return (
    <div className="bg-zinc-800 rounded-xl shadow-lg flex flex-col overflow-hidden w-[110%] ml-[-5%]">
      ...
      <div className="relative h-[158.4px]">
        <div
      ...
      <div className="p-4 flex flex-col grow">
        ...
        <p className="text-sm text-gray-400 mb-4 grow line-clamp-2">
          {course.description}
        </p>
        ...
        <div className="flex items-center justify-between pt-3 border-t border-gray-700/50">
            ...
            <div className="flex items-center space-x-1">
              ...
              >
                <path
                  ...
                  strokeLinejoin="round"
                  strokeWidth={2}
                  d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20v-2a3 3 0 015.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M12 10a2 2 0 100-4 2 2 0 000 4zm-7 12V7.4l2.4-2.4 2.4 2.4V22m-7-9h14"
                />
              </svg>
          ...
          <button
            ...
            className="py-2 px-4 rounded-full font-semibold text-sm text-white bg-blue-600 hover:bg-blue-700 transition"
          >
...
const Learn = () => {
  ...
  const handleAction = async (courseId: string) => {
    const token = localStorage.getItem("token");
    if (!token) return;
    ...
    setCourses((prev) =>
      prev.map((c) =>
        c.id === courseId
          ? { ...c, progress: c.progress && c.progress > 0 ? c.progress : 10 }
          : c
      )
    );
    ...
    try {
      await fetch("/api/courses/progress", {
        ...
        body: JSON.stringify({
          courseId,
          progress: 10, // Default increment for demo
          isBookmarked: true,
        }),
      });
    } catch (error) {
  ...
  if (loading)
    return (
      <div className="min-h-screen bg-zinc-900 text-white p-10 flex justify-center">
        Loading Courses...
      </div>
    );
      ...
      <header className="flex justify-between items-center mb-8">
        ...
        <div className="flex items-center space-x-2 bg-zinc-800 p-2 rounded-lg">
          <svg
            ...
            viewBox="0 0 24 24"
          >
            <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-1 15h2v2h-2v-2zm-1-6h2v4h-2v-4zm-2 2h2v2h-2v-2z" />
          </svg>
          <div>
            <p className="text-xs text-gray-400 leading-none">
              Learning Streak
            </p>
            <p className="text-xl font-bold text-orange-400 leading-none">
              12 days
            </p>
          </div>
        </div>
        ...
        <div className="w-full lg:w-72">
          <div className="bg-zinc-800 p-6 rounded-xl shadow-2xl sticky top-6">
            ...
            <div className="bg-zinc-900 p-3 rounded-lg text-sm">
              <p className="text-white mb-2 leading-snug">
                Hello! I'm your learning assistant. I can help explain concepts,
                provide additional resources, or answer any questions about your
                courses.
              </p>
              <p className="text-gray-500 text-xs text-right">Now</p>
            </div>
          </div>
        </div>
      </div>

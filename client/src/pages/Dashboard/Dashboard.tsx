...
import { 
    LogOut, PenLine, Phone, Mail, Users, Briefcase, Hash, Star, User, 
    ListChecks, Calendar, MessageSquare, Bell, Zap, Bot, AlertTriangle, CheckCircle, 
    Image as ImageIcon, Plus, History, X, Check, Clock, Edit2, Trash2, UserPlus
} from 'lucide-react';
...
const initialTasks: Task[] = [
    {
        ...
        summary: "Final check for compliance and scalability before submission to the board.",
    ...
    },
    {
        ...
        summary: "Migrate existing bash scripts to Ansible playbooks for environment consistency.",
...
const navItems: NavItem[] = [
    ...
    { id: 'comms', label: 'Communication Channel', icon: MessageSquare, color: 'text-purple-400' },
];

const mockScheduleItems: ScheduleItem[] = [
    { id: 's1', title: "Daily Cloud Ops Standup", venue: "Zoom Call (ID: 123-456)", dateTime: "2025-11-20T08:30:00" },
    { id: 's2', title: "Q3 Architecture Review", venue: "Conference Room B", dateTime: "2025-11-20T11:00:00" },
    { id: 's3', title: "1:1 with Senior Manager", venue: "Office 401", dateTime: "2025-11-20T14:00:00" },
    { id: 's4', title: "Security Patch Rollout", venue: "Remote", dateTime: "2025-11-21T09:00:00" },
    { id: 's5', title: "Team Lunch", venue: "The Bistro", dateTime: "2025-11-21T13:00:00" },
    { id: 's6', title: "Database Migration Planning", venue: "MS Teams Meeting", dateTime: "2025-11-22T10:00:00" },
    { id: 's7', title: "Budget Planning Submission", venue: "Finance Portal", dateTime: "2025-11-25T17:00:00" },
    { id: 's8', title: "Monthly Project Sync (Dec)", venue: "Zoom Call (ID: 789-012)", dateTime: "2025-12-01T09:30:00" },
];

const mockDepartmentGroup: ChatGroup = {
    id: 'department',
    name: "Cloud Ops Team: Daily Grind ☁",
    members: ["Aisha Jele (You)", "John Smith", "Jane Doe", "David Chen", "Emily Ross"],
    messages: [
        { id: 1, sender: 'other', name: 'David Chen', text: 'Morning all. Patching report for S03 is ready on the shared drive.', timestamp: '08:15' },
        { id: 2, sender: 'other', name: 'John Smith', text: 'Thanks, David. Need eyes on the Q3 proposal architecture this afternoon.', timestamp: '11:05' },
        { id: 3, sender: 'self', name: 'Aisha Jele', text: 'I have the final compliance check scheduled for 14:00 today. Will provide feedback right after.', timestamp: '11:15' },
        { id: 4, sender: 'other', name: 'Emily Ross', text: 'The new Ansible playbooks are running smoothly. Less manual intervention needed!', timestamp: '16:30' },
        { id: 5, sender: 'self', name: 'Aisha Jele', text: 'Great news, Emily! I\'m updating the documentation now based on the successful runs.', timestamp: '18:10' },
    ],
};
...
const mockCompanyGroup: ChatGroup = {
    ...
    members: ["Aisha Jele (You)", "CEO - Mark T.", "HR - Sarah K.", "Finance - Chris B.", "... and 40 others"],
    messages: [
        { id: 101, sender: 'other', name: 'CEO - Mark T.', text: 'Reminder: All staff meeting tomorrow at 10 AM regarding Q4 strategy.', timestamp: '10:00' },
        { id: 102, sender: 'other', name: 'HR - Sarah K.', text: 'Annual leave forms deadline is next Friday. Please submit on time!', timestamp: '14:45' },
        { id: 103, sender: 'self', name: 'Aisha Jele', text: 'Confirmed attendance for tomorrow\'s strategy meeting.', timestamp: '15:15' },
        { id: 104, sender: 'other', name: 'Finance - Chris B.', text: 'Expense portal is temporarily offline for maintenance until 7 PM.', timestamp: '17:50' },
    ],
};
...
const mockNotifications: Notification[] = [
    { id: 1, message: "Server S01 reached 90% utilization.", type: 'alert', time: '5m ago' },
    { id: 2, message: "Task 'Deployment Scripts' marked complete.", type: 'info', time: '1h ago' },
    { id: 3, message: "New security patch available for review.", type: 'alert', time: '4h ago' },
];
...
const getPriorityColor = (deadline: string): { color: string; label: string } => {
    const today = new Date();
    const deadlineDate = new Date(deadline);
    const diffTime = deadlineDate.getTime() - today.getTime();
    const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24));

    if (diffDays <= 1) return { color: 'border-red-500 bg-red-900/30', label: 'High Priority (Urgent)' };
    if (diffDays <= 7) return { color: 'border-yellow-500 bg-yellow-900/30', label: 'Medium Priority' };
...
const ProfileField: React.FC<ProfileFieldProps> = ({
    ...
    const inputType = label.toLowerCase().includes('email') ? 'email' : 
    ...
    return (
        <div className="flex flex-col space-y-1">
            ...
            {isLocked || !isEditable ? (
                <p className={`text-gray-100 font-medium text-lg ${isLocked ? 'italic' : ''}`}>
                    {value || 'N/A'}
                </p>
            ) : (
                <input
...
const TaskCard: React.FC<TaskCardProps> = ({ task, onComplete, isHistory = false }) => {
    const { color, label } = getPriorityColor(task.deadline);

    return (
        <div className={`p-5 rounded-xl shadow-lg border-l-4 transition transform hover:scale-[1.01] ${color}`}>
            ...
            <div className="text-sm space-y-1">
                <div className="flex items-center text-gray-400">
                    ...
                    <span className="font-semibold">Team:</span> {task.teamMembers || 'Solo'}
                </div>
                <div className="flex items-center text-gray-400">
                ...
                {isHistory && task.dateCompleted && (
                    <div className="flex items-center text-gray-400">
                        ...
                        <span className="font-semibold">Completed:</span> {task.dateCompleted}
                    </div>
                )}
                <div className="text-xs pt-1 text-gray-500">{isHistory ? `Created: ${task.dateCreated}` : `Priority: ${label}`}</div>
            </div>
        </div>
    );
};
...
const TasksComponent: React.FC = () => {
    ...
    const [completedTasks, setCompletedTasks] = useState<Task[]>(completedTasksMock);
    const [viewMode, setViewMode] = useState<'list' | 'add' | 'history'>('list');
    const [newTask, setNewTask] = useState<Omit<Task, 'id' | 'isCompleted' | 'dateCreated'>>({
        title: '',
        summary: '',
        teamMembers: '',
    ...
    const handleAddNewTask = (e: React.FormEvent) => {
        ...
        if (!newTask.title || !newTask.summary || !newTask.deadline) {
            console.error("Validation Error: Title, summary, and deadline are required!"); 
            return;
        }
    ...
    const commonClasses = "p-6 rounded-xl shadow-xl bg-gray-800 flex flex-col h-full";
            ...
            <div className="flex-grow space-y-4 overflow-y-auto pr-2">
                {tasks.length === 0 ? (
                    <div className="text-center p-8 bg-gray-700/50 rounded-lg text-gray-400">
                        <p>No active tasks right now. Click 'Add New Task' to create one!</p>
                    </div>
                ) : (
    ...
    const renderAddTaskForm = () => (
        <>
            ...
            <form onSubmit={handleAddNewTask} className="flex-grow space-y-4 overflow-y-auto pr-2">
                <div>
    ...
    const renderTaskHistory = () => (
        <>
            <h3 className="text-xl font-bold text-yellow-300 mb-4 flex items-center">
                <History className="w-5 h-5 mr-2" /> Task History ({completedTasks.length})
            </h3>
            <div className="flex-grow space-y-4 overflow-y-auto pr-2">
                {completedTasks.length === 0 ? (
                    ...
                    completedTasks.sort((a, b) => new Date(b.dateCompleted || 0).getTime() - new Date(a.dateCompleted || 0).getTime()).map(task => (
                        <TaskCard key={task.id} task={task} onComplete={() => {}} isHistory={true} />
                    ))
                )}
            </div>
        </>
    ...
    return (
        <div className={commonClasses}>
            ...
            <div className="flex-shrink-0 mt-6 pt-4 border-t border-gray-700 flex space-x-4">
                <button
                    ...
                    className={`flex items-center space-x-2 px-4 py-2 text-sm font-semibold rounded-lg transition duration-200 w-1/2 justify-center
                        ${viewMode === 'add' ? 'bg-blue-800 text-white' : 'bg-blue-600 hover:bg-blue-700 text-white'}
                    `}
                ...
                <button
                    ...
                    className={`flex items-center space-x-2 px-4 py-2 text-sm font-semibold rounded-lg transition duration-200 w-1/2 justify-center
                        ${viewMode === 'history' ? 'bg-yellow-800 text-white' : 'bg-yellow-600 hover:bg-yellow-700 text-white'}
                    `}
...
const ScheduleComponent: React.FC = () => {
    ...
    const filterButtonClasses = (isActive: boolean) => 
        `px-4 py-2 text-sm font-semibold rounded-lg transition duration-200 
         ${isActive ? 'bg-cyan-600 text-white shadow-lg' : 'bg-gray-700 text-gray-300 hover:bg-gray-600'}`;
        ...
        <div className={commonClasses + " overflow-y-auto"}>
            ...
            <div className="flex-grow min-h-0 overflow-y-auto pr-2">
                {filteredItems.length === 0 ? (
                    <div className="text-center p-8 bg-gray-700/50 rounded-lg text-gray-400">
                        <p>No upcoming tasks scheduled for *{filter.toUpperCase().replace('WEEK', '7 DAYS').replace('MONTH', '30 DAYS')}*.</p>
                    </div>
                ) : (
                    <table className="min-w-full divide-y divide-gray-700">
                        <thead className="bg-gray-700 sticky top-0">
                            <tr>
                                <th className="px-4 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Task Title</th>
                                <th className="px-4 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider hidden sm:table-cell">Venue/Location</th>
                        ...
                        <tbody className="divide-y divide-gray-700">
                            {filteredItems.map(item => (
                                <tr key={item.id} className="hover:bg-gray-700/50 transition duration-150">
                                    <td className="px-4 py-4 whitespace-nowrap text-sm font-medium text-white">{item.title}</td>
                                    <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-300 hidden sm:table-cell">{item.venue}</td>
                                    <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-300">{formatDate(item.dateTime)}</td>
                                    <td className="px-4 py-4 whitespace-nowrap text-sm font-semibold text-cyan-400">{formatTime(item.dateTime)}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
...
const CommsComponent: React.FC = () => {
    const [activeGroup, setActiveGroup] = useState<'department' | 'company'>('department');
    const chatRef = useRef<HTMLDivElement>(null);

    const currentGroup = mockChatData.find(g => g.id === activeGroup) || mockDepartmentGroup;
    ...
    const buttonClasses = (id: string) => 
        `flex-1 px-3 py-2 text-sm font-semibold rounded-t-lg transition duration-200 text-center 
         ${activeGroup === id ? 'bg-gray-700 text-purple-300 border-b-2 border-purple-500' : 'bg-gray-800 text-gray-400 hover:bg-gray-700/50'}`;
    ...
    return (
        <div className={commonClasses}>
            ...
            <div className="p-3 bg-gray-700/70 border-b border-gray-700 text-xs text-gray-400">
                <span className="font-bold text-gray-200">Members ({currentGroup.members.length}): </span>
                {currentGroup.members.slice(0, 5).join(', ')} 
                {currentGroup.members.length > 5 && '...'}
            </div>

            <div ref={chatRef} className="flex-grow p-4 space-y-4 overflow-y-auto bg-gray-900/50 rounded-b-xl min-h-0">
                {currentGroup.messages.map(msg => (
                    ...
                    >
                        <div className={`max-w-xs md:max-w-md p-3 rounded-xl shadow-lg ${
                            ...
                            {msg.sender === 'other' && (
                                <p className="text-sm font-bold mb-1" style={{ color: msg.name === 'John Smith' ? '#4f46e5' : '#10b981' }}>
                                    {msg.name}
                                </p>
                            )}
                            ...
                            <div className={`text-xs mt-1 ${msg.sender === 'self' ? 'text-gray-200/70' : 'text-gray-400'}`}>
            ...
            <div className="flex-shrink-0 pt-4 flex space-x-2">
                <input 
                    type="text" 
                    placeholder={`Message ${currentGroup.name}...`}
                    className="flex-grow bg-gray-700 text-gray-100 p-3 rounded-lg focus:border-purple-500 focus:ring-1 focus:ring-purple-500 transition"
                />
...
const ContentWindow: React.FC<ContentWindowProps> = ({ selectedItem }) => {
    const commonClasses = "p-6 rounded-xl shadow-xl bg-gray-800 h-full flex flex-col";

    switch (selectedItem) {
        case 'dashboard':
            return (
                <div className={commonClasses + " overflow-y-auto"}>
                    <h3 className="text-xl font-bold text-yellow-300 mb-4">Welcome to Your LightStars Hub</h3>
                    <p className="text-gray-300">
                        This centralized dashboard provides a real-time snapshot of your operational environment. Monitor
                        key metrics, track project progress, and manage your schedule from one interface.
                    </p>
                    ...
                    <div className="mt-8 pt-4 border-t border-gray-700 space-y-3">
                         <h4 className="text-lg font-semibold text-gray-200">Recent Activity Log (Scroll Test)</h4>
                        {Array.from({ length: 20 }).map((_, i) => (
                            <p key={i} className="text-sm text-gray-400">
                                <span className="text-gray-500">[{`2025-11-${20 - (i % 5)} 10:${10 + i} AM`}]</span> 
        ...
        default:
            return <div className={commonClasses}><p className="text-gray-400">Select an item from the navigation bar.</p></div>;
    }
};
...
const ChatbotWindow: React.FC = () => {
    return (
        <div className="p-6 rounded-xl shadow-xl bg-gray-800 h-full flex flex-col border border-blue-700">
            ...
            <div className="flex-grow overflow-y-auto space-y-3 mb-4 text-sm">
                <div className="flex justify-start">
                    <p className="bg-gray-700 p-3 rounded-lg max-w-xs">Hello Aisha! How can I assist with your data architecture or operational tasks today?</p>
                </div>
                <div className="flex justify-end">
                    <p className="bg-blue-600 p-3 rounded-lg max-w-xs">I need a summary of server S01's utilization data.</p>
                </div>
                <div className="flex justify-start">
                    <p className="bg-gray-700 p-3 rounded-lg max-w-xs">Acknowledged. Checking real-time metrics now...</p>
                </div>
            </div>
            <div className="flex space-x-2">
                <input 
                    ...
                    className="flex-grow bg-gray-700 text-gray-100 p-3 rounded-lg focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition"
                />
            ...
            {notifications.length === 0 ? (
                ...
                <ul className="divide-y divide-gray-700">
                    {notifications.map(n => {
                        ...
                        const color = n.type === 'alert' ? 'text-red-400' : 'text-green-400';
                        const bgColor = n.type === 'alert' ? 'hover:bg-red-900/30' : 'hover:bg-green-900/30';
                        
                        return (
                            <li key={n.id} className={`p-4 flex items-start space-x-3 transition ${bgColor} cursor-pointer`}>
...
export default function App() {
    ...
    const handleProfileUpdate = (key: string, newValue: string) => {
        let formattedKey = key.replace(/\s(.)/g, (match, char) => char.toUpperCase());
    ...
    return (
        <div className="flex h-screen bg-gray-900 text-white font-sans overflow-hidden">
            <aside className="w-full sm:w-80 flex-shrink-0 bg-gray-800 border-r border-gray-700 p-6 flex flex-col overflow-y-auto">
                ...
                <div className="flex flex-col items-center mb-6">
                    ...
                    <h1 className="text-2xl font-bold mt-3">{profile.firstName} {profile.lastName}</h1>
                ...
                <div className="space-y-4 flex-grow">
                    <ProfileField icon={Briefcase} label="Company Name" value={profile.companyName} isEditable={false} />
                    <ProfileField icon={User} label="First Name" value={profile.firstName} isEditable={isEditing} onUpdate={handleProfileUpdate} />
                    <ProfileField icon={User} label="Last Name" value={profile.lastName} isEditable={isEditing} onUpdate={handleProfileUpdate} />
                    <ProfileField icon={Users} label="Gender" value={profile.gender} isEditable={isEditing} onUpdate={handleProfileUpdate} />
                    <ProfileField icon={Mail} label="Email" value={profile.email} isEditable={isEditing} onUpdate={handleProfileUpdate} />
                    <ProfileField icon={Phone} label="Phone Number" value={profile.phone} isEditable={isEditing} onUpdate={handleProfileUpdate} />
                    <ProfileField icon={Briefcase} label="Department" value={profile.department} isEditable={isEditing} onUpdate={handleProfileUpdate} />
                    <ProfileField icon={Hash} label="Employee Number" value={profile.employeeNumber} isEditable={isEditing} onUpdate={handleProfileUpdate} />
                    <ProfileField icon={Briefcase} label="Role/Occupation" value={profile.role} isEditable={isEditing} onUpdate={handleProfileUpdate} />
                </div>
            ...
            <main className="flex-grow flex flex-col p-6 sm:p-10 overflow-hidden">
                <header className="flex justify-between items-center mb-6 border-b border-gray-700 pb-3 relative flex-shrink-0">
                ...
                <nav className="flex justify-center space-x-4 mb-8 overflow-x-auto pb-2 flex-shrink-0">
                    {navItems.map((item) => (
                        <button
                            key={item.id}
                            onClick={() => setSelectedNav(item.id)}
                            className={`flex items-center space-x-2 px-6 py-3 rounded-full font-semibold transition duration-200 flex-shrink-0 ${
                                selectedNav === item.id
                ...
                <div className="flex-grow min-h-0">
                    <div className={`grid grid-cols-1 ${isChatOpen ? 'lg:grid-cols-3' : 'lg:grid-cols-3'} gap-6 mb-6`}>
                        <div className="lg:col-span-1 h-110 flex flex-col">
                            <PerformanceTracker score={PERFORMANCE_SCORE} />
                        </div>

                        <div className={`${contentWindowSpan} h-110 flex flex-col overflow-y-auto`}>
            ...
            <div id="logout-modal" className="fixed inset-0 bg-black bg-opacity-70 hidden items-center justify-center z-50">
                <div className="bg-gray-800 p-6 rounded-xl shadow-2xl max-w-sm w-full border border-gray-700">
                    ...
                    <p className="text-gray-300 mb-6">Are you sure you want to log out of LightStars?</p>
